...
const MENU_ITEMS: SubMenuItem[] = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          { label: "Titan X", href: "/solutions/cloud-solutions/titanX-data-transformation" },
          { label: "OmniBot", href: "/solutions/aws-solutions/omnibot-genai" },
          { label: "Opsis Streaming", href: "/solutions/aws-solutions/opsis-live-streaming" },
        ],
      },
...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav className="relative mx-auto flex items-center h-full px-8" onMouseLeave={handleMouseLeave}>
        {/* Logo */}
        <div className="flex items-center">
          <Link href="/" className="flex items-center relative group w-44">
            <Image
              src="/assets/3stLG.png"
              alt="Logo"
              width={50}
              height={40}
              className="h-8 transition-all duration-500 transform group-hover:-translate-x-7"
            />
            <Image
              src="/assets/a.png"
              alt="Logo Alt"
              width={160}
              height={40}
              className="absolute opacity-0 group-hover:opacity-100 transform translate-x-0 group-hover:translate-x-10 transition-all duration-700"
            />
          </Link>
        </div>
        ...
        <ul className="hidden lg:flex absolute left-1/2 -translate-x-1/2 space-x-6 items-center text-white">
          <li>
            <Link href="/" className="flex items-center px-4 py-1 hover:text-green-400 hover:font-bold">
              <FaHome className="mr-2" /> Home
            </Link>
          </li>

          {MENU_ITEMS.map((item, index) => (
            <li key={item.label} className="relative" onMouseEnter={() => handleMouseEnter(index)}>
              <button
              ...
              {openIndex === index && item.children && (
                <div className="absolute top-full mt-4 left-0 bg-[#0b1132] shadow-lg rounded p-4 min-w-[220px]">
                  <ul className="space-y-1">
                    {item.children.map((child) => (
                      <li key={child.label} className="relative">
                        <Link href={child.href} className="block p-2 rounded hover:bg-greenCustomColor2 hover:text-white">
                          {child.label}
                        </Link>
                        ...
                        {child.children && (
                          <ul className="ml-4 mt-1 space-y-1 border-l border-gray-600 pl-3">
                            {child.children.map((sub) => (
                              <li key={sub.label}>
                                <Link href={sub.href} className="block p-2 rounded hover:bg-greenCustomColor2 hover:text-white">
                                  {sub.label}
                                </Link>
                              </li>
                            ))}
        ...
        <div className="hidden lg:flex ml-auto">
          <Link href="/contact-us" className="bg-greenCustomColor2 hover:bg-green-700 text-white px-6 py-2 rounded-full font-semibold transition">
            Contact Us
          </Link>
        </div>

        {/* Mobile toggle */}
        <button className="lg:hidden ml-auto text-white p-4" onClick={toggleMobileMenu} aria-label="Toggle menu">
          {showMobileMenu ? <FaTimes /> : <FaBars />}
        </button>

        {/* Mobile Menu */}
        {showMobileMenu && (
          <div className="lg:hidden absolute top-[60px] left-0 w-full bg-white shadow-lg p-4 z-40">
                              ...
                              <div className="flex justify-between items-center">
                                ...
                                {child.children && (
                                  <button
                                    ...
                                    className="ml-2"
                                  >
                                    {childOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                  </button>
                                )}
                              </div>
